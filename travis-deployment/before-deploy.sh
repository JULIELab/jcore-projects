...
if [ "$TRAVIS_PULL_REQUEST" == 'false' ]; then
    openssl aes-256-cbc -K $encrypted_8ca4d2f260b1_key -iv $encrypted_8ca4d2f260b1_iv -in travis-deployment/codesigning.asc.enc -out travis-deployment/codesigning.asc -d
    if [ ! "$?" -eq "0" ]; then
